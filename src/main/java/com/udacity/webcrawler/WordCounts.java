...
final class WordCounts {
    ...
    static Map<String, Integer> sort(Map<String, Integer> wordCounts, int popularWordCount) {
        ...
        PriorityQueue<Map.Entry<String, Integer>> sortedWordsQueue = new PriorityQueue<>(wordCountsSize, wordCountComparator);
        sortedWordsQueue.addAll(wordCounts.entrySet());
        Map<String, Integer> topWordCount = new LinkedHashMap<>();
        int limitValue = Math.min(popularWordCount, wordCountsSize);
        for (int i = 0; i < limitValue; i++) {
            Map.Entry<String, Integer> entry = sortedWordsQueue.poll();
            assert entry != null;
            topWordCount.put(entry.getKey(), entry.getValue());
        }
        return topWordCount;
    }
